...
map<pair<string, uint32_t>,
		std::chrono::time_point<std::chrono::high_resolution_clock> > startsCK;

std::chrono::time_point<std::chrono::high_resolution_clock> TimeMetrics::Start(
		std::string key) {
...
std::chrono::time_point<std::chrono::high_resolution_clock> TimeMetrics::Stop(
		string key) {
	...
	if (starts.find(key) == starts.end()) {
		throw "Stop Called With Start " + key;
	}
...
std::chrono::time_point<std::chrono::high_resolution_clock> TimeMetrics::Stop(
	...
	if (startsCK.find(key) == startsCK.end()) {
		throw "Stop Called Without Start ";
	}
...
double TimeMetrics::Average(string key) {
	...
	if (durations.find(key) != durations.end()) {
		...
		for_each(begin(list), end(list), [&](double duration) {
			sum += duration;
		});
		average = sum / (double) durations[key].size();
	} else {
		throw "Average Called With No duration list " + key;
	}
	return average;
}
