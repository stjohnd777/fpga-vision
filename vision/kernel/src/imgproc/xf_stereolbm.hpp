...
// clang-format off
    ...
    int x = a - b;
// clang-format off
    #pragma HLS BIND_OP variable=x op=sub
    // clang-format on
    T r;
    if (x > 0) {
        r = x;
...
void xFSADBlockMatching(hls::stream<XF_TNAME(WORDWIDTH_SRC, 1)>& left,
    ...
    if (USE_URAM) {
// clang-format off
        #pragma HLS bind_storage variable=left_line_buf type=RAM_2P impl=URAM
...
// clang-format off
    ...
    if (USE_URAM) {
// clang-format off
        #pragma HLS bind_storage variable=right_line_buf type=RAM_2P impl=URAM
...
// clang-format off
    ...
    if (USE_URAM) {
// clang-format off
        #pragma HLS bind_storage variable=minsad type=RAM_S2P impl=URAM
        // clang-format on
    }
    XF_TNAME(WORDWIDTH_DST, 1) mind[BUF_SIZE];
    if (USE_URAM) {
// clang-format off
        #pragma HLS bind_storage variable=mind type=RAM_S2P impl=URAM
        // clang-format on
    }
    bool skip[BUF_SIZE];
    if (USE_URAM) {
// clang-format off
        #pragma HLS bind_storage variable=skip type=RAM_S2P impl=URAM
        // clang-format on
    }
...
// clang-format off
            ...
            loop_sad_compute:
                ...
                if (USE_URAM) {
// clang-format off
                    #pragma HLS bind_storage variable=skip_val type=RAM_S2P impl=URAM
                    // clang-format on
                }
                int edge_neighbor[BUF_SIZE];
                if (USE_URAM) {
// clang-format off
                    #pragma HLS bind_storage variable=edge_neighbor type=RAM_S2P impl=URAM
                    // clang-format on
                }
                int edge[BUF_SIZE];
                if (USE_URAM) {
// clang-format off
                    #pragma HLS bind_storage variable=edge type=RAM_S2P impl=URAM
                    // clang-format on
                }
                int minsad_p[BUF_SIZE];
                if (USE_URAM) {
// clang-format off
                    #pragma HLS bind_storage variable=minsad_p type=RAM_S2P impl=URAM
                    // clang-format on
                }
                int minsad_n[BUF_SIZE];
                if (USE_URAM) {
// clang-format off
                    #pragma HLS bind_storage variable=minsad_n type=RAM_S2P impl=URAM
                    // clang-format on
                }
...
// clang-format off
    ...
    xf::cv::Sobel<XF_BORDER_CONSTANT, XF_FILTER_3X3, SRC_T, FILTER_T, ROWS, COLS, XF_NPPC1, USE_URAM, XFCVDEPTH_IN,
                  XFCVDEPTH_IN, XFCVDEPTH_IN>(in_mat, in_sobel_x, in_sobel_y);
...
/* This function performs preprocessing and disparity computation for SPC mode */
template <int ROWS,
          int COLS,
          int SRC_T,
          int DST_T,
