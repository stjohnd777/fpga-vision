...
const unsigned char icvSaturate8u_cv1[] = {
...
extern const unsigned char icvSaturate8u_cv1[];
#define CV_FAST_CAST_8U(t) (assert(-256 <= (t) && (t) <= 512), icvSaturate8u_cv1[(t) + 256])
...
// clang-format off
    ...
    XF_DTUNAME(SRC_T, NPC) out_pix[XF_NPIXPERCYCLE(NPC)];
    XF_TNAME(SRC_T, NPC) in_pix;
    XF_TNAME(SRC_T, NPC) HSV_packed;
    XF_TNAME(SRC_T, NPC) RGB[XF_NPIXPERCYCLE(NPC)];
    ap_uint<8> r, g, b;
    int h;
    int s, v;
    ap_uint<8> vmin;
...
// clang-format off
            ...
            in_pix = _src_mat.read(row * (_src_mat.cols >> XF_BITSHIFT(NPC)) + col);

            for (int k = 0; k < XF_NPIXPERCYCLE(NPC); k++) {
                RGB[k] = in_pix.range(k * XF_PIXELWIDTH(SRC_T, NPC) + (XF_PIXELWIDTH(SRC_T, NPC) - 1),
                ...
                out_pix[k].range(7, 0) = (unsigned char)h;
                out_pix[k].range(15, 8) = (unsigned char)s;
                out_pix[k].range(23, 16) = (unsigned char)v;
                HSV_packed.range(k * XF_PIXELWIDTH(SRC_T, NPC) + (XF_PIXELWIDTH(SRC_T, NPC) - 1),
                                 k * XF_PIXELWIDTH(SRC_T, NPC)) = out_pix[k];
            }

            _dst_mat.write(row * (_src_mat.cols >> XF_BITSHIFT(NPC)) + col, HSV_packed);
        }
    }
}
